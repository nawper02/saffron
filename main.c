...
int main(int argc, char* argv[])
{
  ...
  sf_ctx.camera.pos    = (sf_fvec3_t){0.0f, 5.0f, -5.0f};
  ...
  while (running) {
      ...
      sf_put_text(&sf_ctx, "SAFFRON 3D TEST", (sf_ivec2_t){10, 10}, SF_CLR_WHITE, 1);
      char fps_text[32];
      ...
      //for (int i = 0; i < 80; ++i) {
      //  sf_line(&sf_ctx, SF_CLR_RED, (sf_ivec2_t){10*i+(width/2),0}, (sf_ivec2_t){10*i, height});
      //}
      //sf_tri(&sf_ctx, SF_CLR_GREEN, (sf_ivec2_t){0,50}, (sf_ivec2_t){50, 400}, (sf_ivec2_t){200,100});
      //sf_tri(&sf_ctx, SF_CLR_WHITE, (sf_ivec2_t){100,200}, (sf_ivec2_t){300, 10}, (sf_ivec2_t){400,600});
      //sf_rect(&sf_ctx, SF_CLR_RED, (sf_ivec2_t){0,20}, (sf_ivec2_t){20,0});
      //sf_rect(&sf_ctx, SF_CLR_RED, (sf_ivec2_t){20,20}, (sf_ivec2_t){50,50});
      //sf_rect(&sf_ctx, SF_CLR_WHITE, (sf_ivec2_t){50,50}, (sf_ivec2_t){30,30});
      //for (int i = 0; i < 10; ++i) {
      //  sf_put_text(&sf_ctx, "SAFFRON", (sf_ivec2_t){20,105+(40*i)}, SF_CLR_RED, i);
      //}
